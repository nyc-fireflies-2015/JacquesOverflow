...
RSpec.describe User, type: :model do
  context 'contains valid data' do 	
	  ...
	  it { expect(subject).to validate_length_of(:username).is_at_most(50).on(:create) }
	 	it { expect(subject).to validate_length_of(:email).is_at_most(50).on(:create) }
	 	it { expect(subject).to validate_length_of(:avatar_url).is_at_most(50).on(:create) }
 		it { expect(subject).to validate_length_of(:bio).is_at_most(500).on(:create) }
